...
- **`npm run test:native`** — the end-to-end suite again, against Chrome's real implementation (`--enable-features=WebMCP`). This is the one that catches spec-vs-implementation gaps; a shim can only ever prove you agree with yourself. It found that `execute()` receives no options argument in Chrome 152 even though the spec marks it required — which broke *every* custom tool on its first call.
- **`npm run test:e2e`** — 37 assertions across the whole loop: the cold-open demo, propose → dry-run → the drawer's real UI → an in-progress description edit surviving a re-render → a second proposal queueing instead of hijacking the drawer → a real click on Approve → live registration with no reload → the agent calling the tool → idempotency → duplicate-name refusal → Escape-to-dismiss → survives a reload → the injection scanner flagging a poisoned description and credential-shaped schema → overlap detection making consolidation primary → a retire chip freeing the context a tool cost.
